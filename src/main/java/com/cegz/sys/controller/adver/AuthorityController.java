...
/*    @RequiresPermissions(value = {"authority/getAuthorityLists"})
    @PostMapping("getAuthorityLists")
    public ResultData getAuthorityLists(HttpServletRequest request, String version, Integer curPage, Integer pageSize, String name, String phone, String plateNumber, Byte grade){
        if(checkVersion(version) != null){
            return checkVersion(version);
        }
        if(checkToken(request) != null){
        ...
        Long count = authorityService.getAuthorityCounts(name, phone, plateNumber, 4);
        Integer startPage = (curPage - 1) * pageSize;// 计算起始页
        List<Authority> authorityList = authorityService.getAuthorityListByCondition(startPage, pageSize, name, phone, plateNumber, 4);
